...
*   Save and load your projects locally.
*   Edit multiple project files (initially `game.js` and `index.html`).
...
*   In Phaser 3 mode:
    *   Use a **Tabbed interface** to switch between **Live Preview**, **Console Output**, and **Asset Management**.
    *   See a live preview of your running Phaser game code.
    *   View console logs (`log`, `warn`, `error`) generated by your game code directly in the app.
...
*   **Local Project Save/Load:** Save your current code, assets, and settings to a local file and load projects back into the editor.
*   **Basic Multi-File Support:** Edit `game.js` and `index.html` using file tabs.
*   **Dual Mode Operation:**
...
*   **Tabbed Interface (Phaser Mode):** Switch easily between:
    *   **Live Game Preview:** Instantly run and visualize your Phaser code.
    *   **Console Output:** View `console.log/warn/error` messages from your running game.
    *   **Asset Management:** Upload, view, delete, and copy URLs for game assets served locally.
*   **Code Editor:** Integrated CodeMirror editor with JavaScript/HTML syntax highlighting, line numbers, and basic editing features.
*   **Apply AI Suggestions (Hybrid Approach):** Button appears when the AI provides applicable full code modifications, allowing one-click application to the editor.
*   **AI Error Fixing Assist (Phaser Mode):** Detects runtime errors in the game preview and offers a button to prompt the AI for a fix.
...
*   shadcn/ui (UI Components, including Tabs & Resizable Panels)
...
*   Open your web browser and navigate **manually** to the `Local:` URL provided by the "Frontend Server (Vite)" window (e.g., `http://localhost:5173/`).
...
*   **Missing "Run Game" Button:** The button to trigger the game preview within the "Preview" tab (in Phaser Mode) is currently missing due to recent refactoring. This needs to be re-implemented within the `src/components/GamePreview.tsx` component or its parent where the action is triggered.
*   This is an active development project, and features are subject to change and refinement.
